...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// Set the parameters of the sun rays with gentle movement
	float moveX1 = sin(iTime * 0.3) * iResolution.x * 0.08;
	float moveY1 = cos(iTime * 0.4) * iResolution.y * 0.05;
	vec2 rayPos1 = vec2(iResolution.x * 0.7 + moveX1, iResolution.y * 1.1 + moveY1);
	vec2 rayRefDir1 = normalize(vec2(1.0, 0.116));
	float raySeedA1 = 36.2214;
	float raySeedB1 = 21.11349;
	float raySpeed1 = 1.1;

	float moveX2 = cos(iTime * 0.25) * iResolution.x * 0.06;
	float moveY2 = sin(iTime * 0.35) * iResolution.y * 0.04;
	vec2 rayPos2 = vec2(iResolution.x * 0.8 + moveX2, iResolution.y * 1.2 + moveY2);
